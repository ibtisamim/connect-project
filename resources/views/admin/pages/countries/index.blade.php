    ...
    <div class="card">
      <div class="card-body">
        <div class="col-12 d-flex align-items-center justify-content-end pb-1">
          <a href="{{route('countries.create')}}" class="btn btn-sm btn-success">Add Country</a>
        </div>
        <!-- datatable start -->
            ...
            <tbody>
                @foreach ($items as $item)
                    <tr>
                        ...
                        <td>{{$item->country}}</td>
                        <td>
                        <a href="{{route('countries.edit',$item->id)}}" style="display: inline-block; width: auto;padding: 0; border: 1px solid #ccc; padding: 5px; border-radius: 5px; line-height: 12px;"><i class="bx bx-edit-alt"></i></a>
                     
                        <form action="{{route('countries.destroy',$item->id)}}" method="POST" style="display: initial; vertical-align: top;">
                        @csrf
                        @method('delete')
