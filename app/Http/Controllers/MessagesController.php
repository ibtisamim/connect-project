...
class MessagesController extends Controller
{
    ...
    public function getLoadLatestMessages(Request $request)
    {
        ...
        foreach ($messages->reverse() as $message) {
            ...
            $return[] = view('layouts.message-line')->with('message', $message)->render();
    ...
    public function postSendMessage(Request $request)
    {
        ...
        PusherFactory::make()->trigger('chat-message.' . $message->to_user, 'send', $message);
        if(!in_array($message->to_user,Auth::user()->sessionMessages()->pluck("to_user")->toArray())){
            Auth::user()->sessionMessages()->attach(User::find($message->to_user));
        }
    ...
    function getOpenedMessages(Request $request){
        $users = Auth::user()->sessionMessages()->pluck("to_user")->toArray();
        $userData = User::whereIn('id',$users)->get();
        return response()->json(['state' => 1, 'users' => $userData]);
    }

    function getAllMessages(Request $request){
        $messages = Auth::user()->inboxMegs()->get();
        return view('directmessages.index',compact('messages'));
    }
